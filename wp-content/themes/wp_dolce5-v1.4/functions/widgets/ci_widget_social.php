...
class CI_Social extends WP_Widget {
	...
	function __construct(){
		...
		parent::__construct('ci_social_widget', $name='-= CI Social =-', $widget_ops, $control_ops);
	}
	...
	function widget($args, $instance) {
		...
		{
			echo '<a href="'.ci_rss_feed().'" class="s-icon"><span href="'.ci_rss_feed().'" class="icn rss" title="'.ci_setting('social_rss_text').'">'.ci_setting('social_rss_text').'</span></a>';
		}
		...
		foreach($services as $key=>$value)
		{
			...
			{
				echo '<a href="'.$url.'" class="s-icon"><span href="'.$url.'" class="icn '.$key.'" title="'.$text.'">'.$text.'</span></a>';
			}
		}

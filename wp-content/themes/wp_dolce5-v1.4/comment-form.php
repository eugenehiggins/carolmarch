	...
	?>
		<?php if ( comments_open() ) : ?>
			<?php do_action( 'comment_form_before' ); ?>
				 ...
				 <h3 id="reply-title" class="widget-title"><span><?php comment_form_title( $args['title_reply'], $args['title_reply_to'] ); ?><small><?php cancel_comment_reply_link( $args['cancel_reply_link'] ); ?></small></span></h3>
					...
					<?php else : ?>
						<form action="<?php echo site_url( '/wp-comments-post.php' ); ?>" method="post" id="<?php echo esc_attr( $args['id_form'] ); ?>">
							...
							<p>
								<input type="submit" class="fbutton" name="submit" id="<?php echo esc_attr( $args['id_submit'] ); ?>" value="<?php echo esc_attr( $args['label_submit'] ); ?>" />
